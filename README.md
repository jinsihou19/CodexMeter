...
推荐在 GitHub 仓库的 Actions 页面手动运行“打包并发布 CodexMeter”。工作流会自动选择递增构建号、构建 Universal DMG、创建 Release，并把已签名的 `appcast.xml` 部署到 Pages。可选填写新的语义版本；留空则使用工程版本。

首次使用前，需要在仓库 Actions Secret 中配置 `SPARKLE_PRIVATE_KEY`。

本机也可以在工作区已经提交并推送后执行完整发布：
...
```bash
CODEX_RELEASE_VERSION=1.1.0 bash script/package_release.sh
```

需要仅做本地打包和安装、不创建远程 Release 时：

```bash
CODEX_PUBLISH_RELEASE=0 bash script/package_release.sh
```

默认发布模式要求工作区干净，并且本机已安装、登录 [GitHub CLI](https://cli.github.com/)。Actions 和本机发布均使用同一个[发布工作流](.github/workflows/publish-appcast.yml)维护更新源。
