...
/// 持有 Sparkle 标准更新控制器；使用官方界面和校验流程，不在应用内重复实现更新逻辑。
@MainActor
...
/// 设置页的手动更新入口；只触发 Sparkle 标准检查并跟随其可用状态。
...
struct SettingsView: View {
    ...
    /// 关于页集中展示产品身份、当前版本、Sparkle 更新偏好和官方项目入口。
    private var aboutPane: some View {
        VStack(alignment: .leading, spacing: SettingsPanelLayout.sectionSpacing) {
            VStack(spacing: 10) {
                ...
                Text("让 Codex 剩余额度、重置时间和使用节奏一眼可见。")
                    .font(.callout)
                    .foregroundStyle(.secondary)
            }
            ...
            SettingsSection(title: "更新", subtitle: "通过 Sparkle 安全检查和安装 CodexMeter 新版本") {
                SettingsToggleRow(
                    title: "自动检查更新",
                    subtitle: "开启后由 Sparkle 定期检查更新；下载和安装前仍会显示标准确认界面。",
            ...
            SettingsSection(title: "链接", subtitle: "查看源码、版本说明和问题反馈") {
                ...
                Link(destination: URL(string: "https://github.com/jinsihou19/CodexMeter/releases")!) {
                    Label("版本发布记录", systemImage: "shippingbox")
                        .frame(maxWidth: .infinity, alignment: .leading)
                }
                .buttonStyle(.plain)
    ...
    private var widgetPane: some View {
        VStack(alignment: .leading, spacing: SettingsPanelLayout.sectionSpacing) {
            SettingsSection(title: "小组件内容", subtitle: "这些选项会影响所有 CodexMeter 桌面小组件") {
                SettingsPickerRow(
                    title: "显示内容",
