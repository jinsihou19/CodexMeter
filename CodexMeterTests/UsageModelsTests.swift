...
final class UsageModelsTests: XCTestCase {
    /// 验证窗口标签始终来自接口时长，覆盖单窗口 Team 账号的非标准周期。
    func testDurationLabelsUseActualWindowDuration() {
        let hourly = RateLimitWindow(usedPercent: 0, windowDurationMins: 180, resetsAt: nil)
        let daily = RateLimitWindow(usedPercent: 0, windowDurationMins: 43_200, resetsAt: nil)

        XCTAssertEqual(hourly.durationLabel, "3 小时")
    ...
    func testResetCreditsSnapshotSortsByExpirationAndLocalizesStatus() {
        let laterCredit = ResetCreditSnapshot(
            grantedAt: nil,
            expiresAt: Date(timeIntervalSince1970: 3_000),
            status: "available"
        )
        let unknownExpirationCredit = ResetCreditSnapshot(
            grantedAt: nil,
            expiresAt: nil,
            status: "expired"
        )
        let earlierCredit = ResetCreditSnapshot(
            grantedAt: nil,
        ...
        let snapshot = ResetCreditsSnapshot(
            availableCount: 2,
            credits: [laterCredit, unknownExpirationCredit, earlierCredit]
        )

        XCTAssertTrue(snapshot.hasDisplayableContent)
